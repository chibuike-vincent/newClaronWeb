...
export default function UserModal({handleOpen,open,handleClose}) {
    ...
    <div>
      ...
      >
        <Box sx={style} className="modal-wrapper-chat">
            ...
            <h1 className='chat-head-text'>Start New Conversation <span onClick={handleClose}>X</span></h1>
            <input type="text"
            ...
            />
...

             {
                 searchInput.length> 0? filtered.map(patient=>(
                    <div onClick={()=>navigate("/ChatDoctor",{state:{userData: patient}})} class="patient-details">
