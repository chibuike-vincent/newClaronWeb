  ...
  const send = async () => {
    ...
    try {
      ...
      await firebaseApp
        ...
        .doc(chat_code(userData.email, recipientEmail))
        .collection("messages")
        .add(sen);
      ...
      firebaseApp
        ...
        .then((snapshot) => {
          ...
          if (data.token != undefined) {
            console.log(data.token,'vvvvvvv')
            axios
              .post(
                "https://fcm.googleapis.com/fcm/send",
                {
                  "to": data.token,
                  "notification": {
                    "title": `New message from ${userData.firstname} ${userData.lastname}`,
                    "sound": "beep.mp3",
                    "body": "Click to open",
              ...
              console.log(data.token,'jjjjjjjjjj')
          }
  ...
  return (
    <MainLayout>
      <div className="chart-cantaner">
        <h4 className="dr-chat-detail">
          You are Chatting with <span>Dr {state.userData === undefined ? JSON.parse(state.notification.message).sender : state.userData.firstname}</span>
        </h4>
                ...
                <>
                  ...
                  >
                    ...
                    <p className="created-date">
                      {" "}
                      about {timeSince(new Date(chat.createDate))} ago
                    </p>
                  </div>
        ...
        <div className="chat-input-wrapper">
          {loading ? (
            ...
            <div
              className="img_upload_user"
              style={{ display: image ? "block" : "none" }}
            >
              {image !== "" ? (
                <img src={image} alt="" className="upload-image-attachment" />
              ) : (
                ""
              )}
            </div>
          ...
          <div className="actions-wrapper">
            {loading && image == "" ? (
              <img className="loader-img-file" src={loader} alt="" />
            ) : (
              <>
                <input
                  type="file"
            ...
            {loading || image !== "" ? (
              ...
              <label for="file-input" className="send-icon-upload-file">
